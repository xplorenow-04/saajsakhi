  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-lux-text">Categories</h1>
          <p className="text-sm text-lux-muted mt-0.5">Manage product categories</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <p className="text-lux-muted">No categories found</p>
          <button
            onClick={openCreate}
            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.map((category) => (
                  <tr key={category._id} className="border-b border-lux-border/50 hover:bg-lux-bg/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="w-10 h-10 rounded-lg bg-lux-bg flex items-center justify-center overflow-hidden">
                        {category.image?.url ? (
                          <img src={category.image.url} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <Image size={16} className="text-lux-muted" />
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-sm font-medium text-lux-text">{category.name}</p>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs font-mono text-lux-muted">{category.slug}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-sm text-lux-muted truncate max-w-[200px] block">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-lux-card border border-lux-border rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            <h3 className="text-lg font-semibold text-lux-text">Delete Category</h3>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-lux-card border border-lux-border rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-lux-muted mb-1.5">Description (optional)</label>
                 <textarea
              ...
              <div>
                <label className="block text-sm font-medium text-lux-muted mb-1.5">Image (optional)</label>
                <label className="flex items-center gap-3 px-4 py-3 rounded-xl bg-lux-bg border border-lux-border border-dashed cursor-pointer hover:border-gold-500/40 transition-colors">
                  {imagePreview ? (
