  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-lux-text">Orders</h1>
          <p className="text-sm text-lux-muted mt-0.5">Manage customer orders</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          <Filter size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-lux-muted pointer-events-none" />
          <select
            value={statusFilter}
            onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }}
            className="bg-lux-card border border-lux-border rounded-xl pl-9 pr-8 py-2.5 text-sm text-lux-text focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/30 transition-all appearance-none cursor-pointer"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <Eye className="mx-auto h-12 w-12 text-lux-muted/40 mb-4" />
          <p className="text-lux-muted">No orders found</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              ...
              <tbody>
                {orders.map((order) => (
                  <tr
                    key={order._id}
                    className="border-b border-lux-border/50 hover:bg-lux-bg/30 transition-colors"
                  >
                    <td className="px-4 py-3">
                      <span className="text-xs font-mono text-lux-muted">
                        #{order._id?.slice(-8)}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-sm text-lux-text truncate max-w-[140px]">
                        {order.shippingAddress?.name || order.user?.name || "N/A"}
                      </p>
                      <p className="text-xs text-lux-muted truncate max-w-[140px]">
                        {order.user?.email || ""}
                      </p>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-[11px] font-medium px-2 py-0.5 rounded-full capitalize ${
                          statusColors[order.orderStatus] || "text-lux-muted bg-surface-600"
                        }`}
                      >
                        {order.orderStatus}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          className="bg-lux-bg border border-lux-border rounded-lg px-2 py-1.5 text-xs text-lux-text focus:outline-none focus:border-gold-500 transition-colors disabled:opacity-50 cursor-pointer"
                        >
                          {statusOptions.map((s) => (
                            <option key={s} value={s} className="capitalize">{s}</option>
                          ))}
      ...
      {cancelConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-lux-card border border-lux-border rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            <h3 className="text-lg font-semibold text-lux-text">Delete Order</h3>
            <p className="text-sm text-lux-muted mt-2">Are you sure you want to delete this order? This action cannot be undone.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-10 pb-10 px-4 overflow-y-auto">
          ...
          <div className="relative bg-lux-card border border-lux-border rounded-2xl w-full max-w-2xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-lux-border">
              <h2 className="text-lg font-semibold text-lux-text">Create Manual Order</h2>
              <button
            ...
            <form onSubmit={handleCreateManualOrder} className="p-6 space-y-6">
              <div>
                <h3 className="text-sm font-semibold text-lux-text mb-3 flex items-center gap-2">
                  <User size={16} className="text-gold-400" />
                  Customer Details
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <label className="block text-sm font-medium text-lux-muted mb-1.5">Full Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-lux-muted mb-1.5">Phone</label>
                    <input
                      type="tel"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-lux-muted mb-1.5">City</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-lux-muted mb-1.5">Pincode</label>
                    <input
                      type="text"
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-sm font-medium text-lux-muted mb-1.5">Address</label>
                    <textarea
              ...
              <div>
                ...
                <div className="relative mb-3">
                  ...
                  <input
                    ...
                    className="w-full bg-lux-bg border border-lux-border rounded-xl pl-10 pr-4 py-2.5 text-sm text-lux-text placeholder-text-muted focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500/30 transition-all"
                  />
                  {searchingProducts && (
                ...
                {productResults.length > 0 && (
                  <div className="mb-3 bg-lux-bg border border-lux-border rounded-xl max-h-48 overflow-y-auto">
                      ...
                      >
                        <div className="w-8 h-8 rounded-lg bg-surface-600 overflow-hidden shrink-0">
                          {product.images?.[0] ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-lux-muted text-xs">N/A</div>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-lux-text truncate">{product.name}</p>
                          <p className="text-xs text-lux-muted">â‚¹{product.price} | {product.category}</p>
                        </div>
                ...
                {selectedProducts.length > 0 && (
                  <div className="space-y-2">
                    {selectedProducts.map((product) => (
                      <div key={product._id} className="flex items-center gap-3 bg-lux-bg rounded-xl px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-lux-text truncate">{product.name}</p>
                          <p className="text-xs text-lux-muted">â‚¹{product.price}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <select
                            value={product.size}
                            onChange={(e) => updateSelectedProduct(product._id, "size", e.target.value)}
                            className="bg-surface-600 border border-surface-500 rounded-lg px-2 py-1.5 text-xs text-lux-text focus:outline-none focus:border-gold-500 transition-colors cursor-pointer"
                          >
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="w-7 h-7 rounded-lg bg-surface-600 text-lux-muted hover:text-lux-text transition-colors flex items-center justify-center"
                            >
                              -
                            </button>
                            <span className="w-7 text-center text-sm text-lux-text">{product.quantity}</span>
                            <button
                              type="button"
                          ...
                          <button
                            ...
                            className="p-1.5 text-lux-muted hover:text-danger transition-colors"
                          >
                            <X size={15} />
                          </button>
                        </div>
