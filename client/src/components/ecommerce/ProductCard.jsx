...
export default function ProductCard({ product, loading = false }) {
  ...
  return (
    <Link
      to={`/shop/${slug || _id}`}
      className="group block bg-surface-800 rounded-xl overflow-hidden border border-surface-700/50 hover:border-surface-600 transition-all duration-300 hover:shadow-xl hover:shadow-black/20 hover:-translate-y-0.5"
    >
      {/* Image Container */}
      <div className="relative aspect-[4/5] overflow-hidden bg-surface-700">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
          <button
            ...
            className="bg-white text-surface-900 p-3 rounded-full hover:bg-accent hover:text-white transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
          >
            <ShoppingBag size={18} />
          </button>
          <div className="bg-white/20 backdrop-blur-sm text-white p-3 rounded-full hover:bg-white/30 transition-all duration-200 shadow-lg">
            <Eye size={18} />
          </div>
        </div>
        ...
        {/* Category Tag */}
        {displayCategory && (
          <div className="absolute top-3 right-3 bg-surface-900/70 backdrop-blur-sm text-text-muted text-[10px] font-medium px-2.5 py-1 rounded-md uppercase tracking-wider">
            {displayCategory}
          </div>
        )}
      </div>
      ...
      <div className="p-4 space-y-3">
                ...
                <button
                  ...
                  className={`px-2.5 py-1 text-[11px] font-medium rounded-md border transition-all ${
                    isSelected
                      ...
                      : outOfStock
                      ? "bg-surface-700/50 border-surface-600 text-text-muted/40 line-through cursor-not-allowed"
                      : "bg-surface-700 border-surface-600 text-text-muted hover:border-accent hover:text-accent"
                  }`}
                >
                  {sizeValue}
                </button>
        ...
        <button
          onClick={handleAddToCart}
          disabled={adding || !defaultSize}
          className="lg:hidden w-full flex items-center justify-center gap-2 bg-accent hover:bg-accent/90 disabled:bg-surface-600 disabled:text-text-muted text-white text-sm font-medium py-2.5 rounded-lg transition-all"
        >
          <ShoppingBag size={16} />
